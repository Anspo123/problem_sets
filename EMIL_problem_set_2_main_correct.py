...
class DataAnalysis:
    ...
    def descriptive_statistics_by_assignment(
        self,
        variables,
        field_variable,
        group_variable,
        ...
        """Save numeric statistics and field distributions by assignment."""
        ...
        sections = [
            "DESCRIPTIVE STATISTICS BY ASSIGNMENT STATUS",
            "=" * 60
        ]

        for group_value, group_df in self.df.groupby(group_variable):
            group_name = group_names.get(
                group_value,
                f"{group_variable} = {group_value}"
            )
        ...
        field_counts = pd.crosstab(
            self.df[field_variable],
            self.df[group_variable]
        ).rename(columns=group_names)
        ...
        field_table = pd.concat(
            [field_counts, field_percentages],
            axis=1
        )
...
def main():
    ...
    # File paths:
    input_path = Path("problem_set_2") / "input"
    output_path = Path("problem_set_2") / "output"
    ...
    # -----------------------
    # field is categorical and is therefore reported using counts and
    # percentages instead of means and standard deviations.
    ...
    analyze_data.descriptive_statistics_by_assignment(
        variables=descriptive_variables,
        field_variable="field",
        group_variable="assigned",
        output_path=(
            output_path /
            "descriptive_statistics_by_assignment.txt"
            )
        )
